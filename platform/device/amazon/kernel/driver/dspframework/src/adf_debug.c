...
static adf_mem_cpy adf_dbg_mem_write = NULL;
				...
				const char __user *userbuf, size_t count, loff_t *ppos)
{
	...
	if (adfDspPriv) {
		mutex_lock(&adfDspPriv->adfDspLock);
		if (adfDspPriv->dspHeader == NULL)
			pr_warning("Invalid dspHeader???\n");
		else if (adfDspPriv->blockDbg != 0) {
			if (printk_ratelimit())
				pr_warning("dsp %d is blocked for debug channel %s, %d\n",
					dspId, __FILE__, __LINE__);
		} else if (adfDspPriv->dspHeader->magic == ADF_FW_MAGIC) {
		...
		if (adfDspPriv->dspHeader == NULL)
			...
			if (kstrtou8(buf, 16, &sta) == 0) {
				...
				ret = adf_dbg_mem_write((uintptr_t)staInfo->addr,
					(uintptr_t)staInfo->data, staInfo->size);
				if (ret)
					pr_err("State write to dsp fail!\n");
				else
					pr_info("Set dsp state to 0x%02x\n", sta);
			} else {
				pr_warning("Invalid state input!\n");
			}
		}
		mutex_unlock(&adfDspPriv->adfDspLock);
	}
				...
				void *logFliter)
{
	...
	if (adfDspPriv) {
		mutex_lock(&adfDspPriv->adfDspLock);
		if (adfDspPriv->dspHeader == NULL)
			pr_warning("Invalid dspHeader %s, %d\n", __FILE__, __LINE__);
		else if (adfDspPriv->blockDbg != 0) {
			if (printk_ratelimit())
				pr_warning("dsp %d is blocked for debug channel %s, %d\n",
					dspId, __FILE__, __LINE__);
			...
			if (dspState < ADF_STATE_RUN || dspState >= ADF_STATE_ERR) {
				pr_err("Invalid ADF state 0x%02x\n", dspState);
				mutex_unlock(&adfDspPriv->adfDspLock);
				return ret;
			}
			...
			for (i = 0; i < cliCmdNum; i++) {
				...
				if (adf_dbg_mem_read((uintptr_t)cliInfo->addr,
					...
					pr_err("Can't send adf cli command\n");
			}
		...
		if (adfDspPriv->dspHeader == NULL)
			...
			if (staInfo) {
				...
				if (ret)
					...
					pr_err("Failed to read state from dsp!\n");
			}
		}
		mutex_unlock(&adfDspPriv->adfDspLock);
	}
...
void adfDebug_printLog(uint8_t dspId, uint32_t mode)
{
	...
	if ((adfDspPriv == NULL) || (adfDspPriv->dspHeader == NULL)) {
		pr_err("dsp %d, mode %d, invalid dspHeader\n", dspId, mode);
		return ;
	}

	/* confirm whether it's ADF header */
	if (adfDspPriv->dspHeader->magic != ADF_FW_MAGIC) {
		pr_err("dsp %d, mode %d, not ADF header, magic %x\n",
			dspId, mode, adfDspPriv->dspHeader->magic);
		return ;
	}

	mutex_lock(&adfDspPriv->adfDspLock);

	/* do not print log unless DSP is up and running */
	if (adfDspPriv->blockDbg != 0) {
		if (printk_ratelimit())
			pr_warning("dsp %d is blocked for debug channel %s, %d\n",
				dspId, __FILE__, __LINE__);
		mutex_unlock(&adfDspPriv->adfDspLock);
...
static int _adfDebug_logDumpHandler(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		for (i = 0; i < DSP_CORE_NUM; i++) {
			/* only dump after dsp enter run state all init finished */
			if (adfDebug_queryState(i) == ADF_STATE_RUN)
				adfDebug_printLog(i, ADF_LOG_DUMP_RECENT);
		}
	}
...
EXPORT_SYMBOL_GPL(adfDebug_initLogDumpThread);

/*
 * adfDebug_blockDbg()
 * ----------------------------------------
 * When DSP is crashed, then we have to block the debug channel until
 * DSP is resumed, both CLI and LOG and STA
 *
 * Input:
 *   uint8_t dspId         - the index of the DSP core
 *   uint32_t opt          - the option, 0 for unblock and 1 for block
 * Return:
 *   None
 */
void adfDebug_blockDbg(uint8_t dspId, uint32_t opt)
{
	...
	/* confirm whether the core index is valid, and we can get the priv */
	if ((adfDspPriv == NULL) || (adfDspPriv->dspHeader == NULL)) {
		pr_err("Error! dsp %d, invalid dspHeader\n", dspId);
		return ;
	}

	/* confirm whether it's ADF header */
	if (adfDspPriv->dspHeader->magic != ADF_FW_MAGIC) {
		pr_err("Error! dsp %d, not ADF header, magic %x\n",
			dspId, adfDspPriv->dspHeader->magic);
		return ;
	}

	mutex_lock(&adfDspPriv->adfDspLock);
	adfDspPriv->blockDbg = opt;
	mutex_unlock(&adfDspPriv->adfDspLock);
}

EXPORT_SYMBOL_GPL(adfDebug_blockDbg);
