...
static char *adfLog_allocAndReadRbuf(adfRingbuf_t *rbuf, int32_t *size)
{
	...
	if ((len == ADF_LOG_CYCLE_DUMP_LEN) && (buf[len - 1] != '\n')) {
		...
		buf[len] = '\0';
		/* If there is no "\n" in 2K memory log buffer. The log is abnormal and should be flushed. */
		if (len > 0)
			*size = len;
		else
			pr_warn("%s invalid log because single line > %d characters\n",
				__func__, ADF_LOG_CYCLE_DUMP_LEN);
	}
	adfRingbuf_flush(rbuf, *size);
