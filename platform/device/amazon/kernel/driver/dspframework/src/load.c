...
adfFwHdr_secInfo_t *adfLoad_GetSecInfo(adfFwHdr_t *dspHeader, char *secName)
{
	...
	if (dspHeader) {
		...
		if (i >= dspHeader->seccnt) {
			ADF_LOG_E(TAG, "can't find section info\n");
			return NULL;
		}
	}
	return info;
		...
		if (strcmp(info->name, "BIN") == 0) {
			...
			if (binCpy) {
				...
				goto ERROR;
					...
					(strcmp(info->name, "CFG") == 0) ||
					(strcmp(info->name, "LOG") == 0)) {
			/* malloc data for STA CLI CFG LOG the first time we load dsp fw */
			info->data = NULL;
			...
			} else if (strcmp(info->name, "CFG") == 0) {
				if (adfDspPriv->cfgData == NULL)
					adfDspPriv->cfgData = (uint32_t *)kmalloc(info->size, GFP_KERNEL);
				info->data = adfDspPriv->cfgData;

				/* init the CFG section, then we'll write to RAM to config DSP */
				memset(info->data, 0, info->size);
				if (binCpy) {
					dstAddr = (uint8_t *)((uintptr_t)hostDst + (info->addr - baseAddr));
					if (binCpy(dstAddr, info->data, info->size)) {
						ADF_LOG_I(TAG, "Copy CFG section to %p, len %x, addr %x\n",
							dstAddr, info->size, info->addr);
					}
				}
			} else {
...
ERROR:
	...
	if (adfDspPriv->dspHeader != NULL) {
		...
		/* staData/cliData/cfgData/logData has been assigned to the related
