...
#define MMC_MTK_SIP_CRYPTO_CTRL BIT(0)
				     ...
				     int slot)
{
	...
	if (!(cqhci_readl(host, CQHCI_CFG) & CQHCI_CRYPTO_ENABLE)) {
		arm_smccc_smc(MTK_SIP_KERNEL_HW_FDE_MSDC_CTL,
			      MMC_MTK_SIP_CRYPTO_CTRL, 0, 0, 0, 0, 0, 0,
			      &smccc_res);
		if (!(cqhci_readl(host, CQHCI_CFG) & CQHCI_CRYPTO_ENABLE)) {
			pr_info("Cannot enable eMMC crypto in atf!\n");
			WARN_ON(1);
		}
	}
