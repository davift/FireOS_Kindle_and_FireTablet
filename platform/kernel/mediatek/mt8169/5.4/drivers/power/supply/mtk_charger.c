		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		ret = kstrtouint(buf, 0, &custom_charging_mode);
		if (ret == 0)
			info->custom_charging_mode = custom_charging_mode;
		else
...
static bool get_bcm_flag(struct mtk_charger *info)
{
	...
	if (info->bcm_flag == BCM_FLAG_CHARGE_TIMER)
		...
		chr_err("%s: Charging is stopped by BCM:<secondary_thermal>\n",
		         __func__);
...
stop_charging:
	...
	chr_err("[%s] tmp:%d (jeita:%d sm:%d cv:%d en:%d) (sm:%d) en:%d c:%d s:%d b:%d ov:%d e:%d %d i:%d %d %d\n",
		__func__, temperature, info->enable_sw_jeita, info->sw_jeita.sm,
		info->sw_jeita.cv, info->sw_jeita.charging, thermal->sm,
		charging, info->cmd_discharging, info->safety_timeout, info->bcm_flag,
