...
void select_cv(struct mtk_charger *info)
{
	...
	if (info->custom_charging_mode != 0 && constant_voltage > DEFAULT_DEMO_CHARGING_CV)
...
static bool is_usb_plugout_or_disabled(struct mtk_charger *info)
{
	int ret;
	bool is_en = true;
	...
	if (ret < 0) {
		pr_err("%s: get power path failed\n", __func__);
		return true;
	}
	if (get_charger_type(info) == POWER_SUPPLY_TYPE_UNKNOWN || !is_en) {
		pr_err("%s:USB invalid when type %d powerpath %d\n", __func__,
			get_charger_type(info), is_en);
		return true;
	}
	return false;
}
...
static bool adapter_power_detection_by_ocp(struct mtk_charger *info)
{
	...
	for (i = 0; i < 5; i++) {
		msleep(10);
		if (is_usb_plugout_or_disabled(info))
			goto abort_run_aicl;
	}
...
abort_run_aicl:
	/* Restore IUB/ICHG/CV setting, set MIVR to inital setting */
...
static int adapter_power_detection(struct mtk_charger *info)
{
	...
	if (!adapter_power_detection_by_ocp(info)) {
		pr_err("%s:USB invalid, rst check result\n", __func__);
		det->iusb_ua = 0;
		pdata->input_current_limit_by_aicl = -1;
		return -ENODEV;
	}

	aicl_ua = pdata->input_current_limit_by_aicl;
	if (aicl_ua < 0) {
...
static void check_invalid_charger(struct mtk_charger *info)
{
	...
	charger_dev_set_mivr(info->chg1_dev, info->data.min_charger_voltage);
	charger_dev_set_input_current(info->chg1_dev, 500000);
	pr_info("%s: ichg %d, vbus %d, vdpm %d, idpm %d\n", __func__,
		info->invchg_check_iusb_ua, vbus_uv, vdpm, idpm);
	...
	if (info->invalid_charger_det_weak) {
		if (!is_usb_plugout_or_disabled(info)) {
			pr_err("%s: found invalid charger\n", __func__);
