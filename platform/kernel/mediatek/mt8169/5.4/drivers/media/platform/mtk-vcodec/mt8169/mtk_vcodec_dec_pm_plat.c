...
#define CODEC_VDEC_FREQ 416
#define DEC_ACCELERATE 1
...
bool mtk_vdec_need_accelerate(struct mtk_video_fmt *fmt, unsigned long current_freq_mhz)
{
	bool bNeed = false;
#if DEC_ACCELERATE
	mtk_v4l2_debug(8, "current_freq_mhz:%lu,%d, fourcc:%d, VP8:%d\n",
		CODEC_VDEC_FREQ, current_freq_mhz, fmt->fourcc, V4L2_PIX_FMT_VP8);
	if (((fmt->fourcc == V4L2_PIX_FMT_VP8) ||
		(fmt->fourcc == V4L2_PIX_FMT_MPEG4) ||
		(fmt->fourcc == V4L2_PIX_FMT_H263) ||
		(fmt->fourcc == V4L2_PIX_FMT_MPEG2)) &&
		(current_freq_mhz < CODEC_VDEC_FREQ)) {
		mtk_v4l2_debug(8, "current_freq_mhz:%lu, Need accelerate!\n", current_freq_mhz);
		bNeed = true;
	}
#endif
	mtk_v4l2_debug(8, "current_freq_mhz:%lu, bNeed:%d,%d\n", current_freq_mhz,bNeed, (current_freq_mhz < CODEC_VDEC_FREQ));
	return bNeed;
}
void mtk_vdec_dvfs_begin(struct mtk_vcodec_ctx *ctx, int hw_id)
{
	...
	if (ctx->dec_params.operating_rate > 0) {
		target_freq_mhz = 312L *
				...
				3840L / 2160L / 30L;
		if (mtk_vdec_need_accelerate(ctx->q_data[MTK_Q_DATA_SRC].fmt, target_freq_mhz)) {
			target_freq_mhz = CODEC_VDEC_FREQ;
		}
		...
		target_freq_mhz = target_freq_hz / 1000000UL;
		if (mtk_vdec_need_accelerate(ctx->q_data[MTK_Q_DATA_SRC].fmt, target_freq_mhz)) {
			target_freq_hz = CODEC_VDEC_FREQ  * 1000000UL;
		}
		if (ctx->dev->vdec_reg != 0) {
			opp = dev_pm_opp_find_freq_ceil(&ctx->dev->plat_dev->dev,
						&target_freq_hz);
