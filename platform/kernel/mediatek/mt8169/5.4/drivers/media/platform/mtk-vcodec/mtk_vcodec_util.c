...
void  mtk_vcodec_calculate_ctx_list(struct mtk_vcodec_ctx *ctx)
{
	struct mtk_vcodec_ctx *ctx_iterator;
	unsigned int area = 0;
	int total_rate = 0;
	...
	list_for_each_entry(ctx_iterator, &ctx->dev->ctx_list, list) {
		frame_rate = ctx_iterator->enc_params.framerate_num /
				ctx_iterator->enc_params.framerate_denom;
		area += ctx_iterator->q_data[MTK_Q_DATA_SRC].visible_width *
			ctx_iterator->q_data[MTK_Q_DATA_SRC].visible_height *
			frame_rate;
		total_rate += frame_rate;
		inst_num++;
	}

	mtk_v4l2_debug(1, "total_calculation=%u area=%u total_fps=%d total_rate=%d ctx=%p ctx_id=%d",
			p->total_calculation,
			area, p->total_fps, total_rate, ctx, ctx->id);

	if (p->total_calculation != area ||
		p->total_fps != total_rate) {
		ctx->param_change |= MTK_ENCODE_PARAM_TOTAL_CALCULATION;
		p->total_calculation = area;
		p->total_fps = total_rate;
		mtk_v4l2_debug(1, "update p->total_calculation is %d, %d ctx=%p ctx_id=%d\n",
				p->total_calculation,
				p->total_fps, ctx, ctx->id);
	}
	mutex_unlock(&ctx->dev->enc_dvfs_mutex);
}
EXPORT_SYMBOL_GPL(mtk_vcodec_calculate_ctx_list);
