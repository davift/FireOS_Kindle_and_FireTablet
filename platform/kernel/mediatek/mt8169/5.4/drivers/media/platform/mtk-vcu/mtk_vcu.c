...
static void vcu_set_gce_cmd(struct cmdq_pkt *pkt,
	struct mtk_vcu *vcu, int gce_index, unsigned int gce_order, struct mtk_vcu_queue *q,
	...
	struct mtk_vcu_queue *q, unsigned long arg)
{
	...
	gce_idx = vcu_gce_get_inst_id(vcu, buff.cmdq_buff.gce_handle);
	...
	if (buff.cmdq_buff.codec_type == VCU_VENC &&
		...
		do {
			if (vcu->cbf.enc_lock != NULL) {
				vcu->cbf.enc_lock(vcu->gce_info[gce_idx].v4l2_ctx, core_id, &lock);
				if (lock != 0) {
	...
	memcpy(&vcu->gce_info[gce_idx].buff[gce_order], &buff, sizeof(buff));

	pkt_ptr->err_cb.cb =
		(buff.cmdq_buff.secure == 0)?vcu_gce_timeout_callback:NULL;
	pkt_ptr->err_cb.data = (void *)&vcu->gce_info[gce_idx].buff[gce_order];
	...
	cmdq_pkt_flush_threaded(pkt_ptr,
		vcu_gce_flush_callback, (void *)&vcu->gce_info[gce_idx].buff[gce_order]);
...
unsigned int vcu_get_vdec_hw_capa(struct platform_device *pdev)
{
	...
	if ((vcu == NULL) || (vcu_running(vcu) == false)) {
		dev_info(&pdev->dev, "vcu device in not ready %p\n", vcu);
		return 0;
	}
...
static int vcu_alloc_d_ext_mem(struct mtk_vcu *vcu, unsigned long len)
{
	...
	dev_dbg(vcu->dev,
		"[VCU] Data extend memory (len:%lu) virt=%p\n",
		VCU_DMEM0_LEN(vcu),
		VCU_DMEM0_VIRT(vcu));
	return 0;
}
...
static int mtk_vcu_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (vcu_queue->map_buf != 1) {
		...
		if (pa_start >= MAP_SHMEM_COMMIT_BASE &&
			...
			vcu_dbg_log("[VCU] map SHMEM_COMMIT, pa_start=0x%lx\n", vma->vm_start);
			goto valid_map;
		}
	}
...
gce_valid_map:
	if (remap_pfn_range(vma, vma->vm_start, vma->vm_pgoff,
			    ...
			    vma->vm_page_prot) != 0) {
		pr_err("[VCU] remap_pfn_range reg memory.\n");
		return -EAGAIN;
	}
	return 0;

valid_map:
	dev_dbg(vcu_dev->dev, "[VCU] start:0x%x\n", vma->vm_start);
	iRet = remap_vmalloc_range(vma, VCU_DMEM0_VIRT(vcu_dev), 0);
	if (iRet) {
