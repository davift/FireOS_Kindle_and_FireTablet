...
static int vidioc_venc_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_MPEG_MTK_SEC_ENCODE:
		...
		mtk_v4l2_debug(2, "[%d] V4L2_CID_MPEG_MTK_SEC_ENCODE id %d val %d array[0] %d array[1] %d",
			ctx->id, ctrl->id, ctrl->val,
		ctrl->p_new.p_u32[0], ctrl->p_new.p_u32[1]);
		break;
			       ...
			       struct venc_enc_param *param)
{
	...
	param->nonrefpfreq = enc_params->nonrefpfreq;
...
static int vb2ops_venc_start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	if (param.nonrefp != 0) {
		ret = venc_if_set_param(ctx, VENC_SET_PARAM_NONREFP, &param);
		if (ret) {
			mtk_v4l2_err("venc_if_set_param failed=%d", ret);
			ctx->state = MTK_STATE_ABORT;
			mtk_venc_queue_error_event(ctx);
	...
	if (param.nonrefpfreq != 0) {
		ret = venc_if_set_param(ctx, VENC_SET_PARAM_NONREFPFREQ, &param);
		if (ret) {
			mtk_v4l2_err("venc_if_set_param failed=%d", ret);
			ctx->state = MTK_STATE_ABORT;
			mtk_venc_queue_error_event(ctx);
...
static int mtk_venc_param_change(struct mtk_vcodec_ctx *ctx)
{
	...
	if (!ret &&
	mtk_buf->param_change & MTK_ENCODE_PARAM_TOTAL_CALCULATION) {
		enc_prm.total_calculation =
			mtk_buf->enc_params.total_calculation;
		enc_prm.total_fps = mtk_buf->enc_params.total_fps;
		mtk_v4l2_debug(1, "[%d] idx=%d, total_calculation=%d,%d",
				ctx->id,
				mtk_buf->vb.vb2_buf.index,
				mtk_buf->enc_params.total_calculation,
				mtk_buf->enc_params.total_fps);
		ret |= venc_if_set_param(ctx,
					VENC_SET_PARAM_TOTAL_CALCULATION,
					&enc_prm);
	}
