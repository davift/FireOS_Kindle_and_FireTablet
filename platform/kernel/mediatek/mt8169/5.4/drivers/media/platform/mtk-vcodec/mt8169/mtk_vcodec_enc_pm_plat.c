...
void mtk_venc_dvfs_begin(struct mtk_vcodec_ctx *ctx, int core_id)
{
	...
	mtk_vcodec_calculate_ctx_list(ctx);

	mutex_lock(&ctx->dev->enc_dvfs_mutex);
	total_rate = ctx->enc_params.total_fps;
	area = ctx->enc_params.total_calculation;

	if (area >= 1920 * 1080 * 60 || total_rate >= 240)
		idx = 2;
	else if (area > 1920 * 1080 * 30 || total_rate >= 120)
		idx = 1;
	else
		idx = 0;
	...
	if (venc_cur_job != 0) {
		...
		target_freq_hz = (ctx->dev->venc_freq.active_freq > target_freq_hz) ?
			ctx->dev->venc_freq.active_freq : target_freq_hz;
...
void mtk_venc_dvfs_end(struct mtk_vcodec_ctx *ctx, int core_id)
{
	...
	target_freq_hz = ctx->dev->venc_freq.active_freq;
	if (ctx->dev->venc_reg != 0) {
		opp = dev_pm_opp_find_freq_ceil(&ctx->dev->plat_dev->dev, &target_freq_hz);
		volt = dev_pm_opp_get_voltage(opp);
		dev_pm_opp_put(opp);
