...
int cmdq_pkt_realloc_cmd_buffer(struct cmdq_pkt *pkt, size_t size)
{
	...
	while (pkt->buf_size < size) {
		...
		if (status) {
			cmdq_err("realloc cmd buffer failed!");
			return (int)status;
		}
	}
	return 0;
...
u64 *cmdq_pkt_get_va_by_offset(struct cmdq_pkt *pkt, size_t offset)
{
	...
	cmdq_err("%s get va failed!", __func__);
...
{
#define txt_len 128
#define txt_gpr_len 30
	char text[txt_len];
	char text_gpr[txt_gpr_len] = {0};
	void *base = NULL;
	u32 gprid = 0, val = 0;
	s32 write_count = 0;
	...
	if (inst->arg_a >= CMDQ_EVENT_GPR_TIMER &&
		...
		write_count = snprintf(text_gpr, txt_gpr_len,
			" GPR R%u:%#x", gprid, val);
		if (write_count <= 0 || write_count >= txt_gpr_len)
			cmdq_err("%s %d snprintf fail!", __func__, __LINE__);
	}
