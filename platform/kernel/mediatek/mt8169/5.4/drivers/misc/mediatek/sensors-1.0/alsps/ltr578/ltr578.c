...
static ssize_t ltr578_show_cali_Light_High(struct device_driver *ddri, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE,
				"%s:Threshold = %d, als_value = %d, alsval = %d, clearval = %d\n",
				result ? "PASSED" : "FAIL", ltr578_obj->lux_threshold, als_reading, raw_alsvalue_reading, raw_clearvalue_reading);
#else
	return scnprintf(buf, PAGE_SIZE,
...
static int als_get_data(int *value, int *status)
{
	...
	if (res < 0) {
		...
		*value = cali_lux;
		...
		if (*value < 0) {
			*value = 0;
			APS_LOG("cali_lux = %d, set value to 0\n", cali_lux, *value);
		}
				...
				const struct i2c_device_id *id)
{
	...
	err = of_property_read_u32_array(client->dev.of_node, "ratio", obj->ratio, RATIO_RANGE - 1);
	if (err) {
		APS_ERR("get ratio info from dts failed\n");
		obj->ratio[0] = 89;
		obj->ratio[1] = 126;
		obj->ratio[2] = 300;
	}
	err = of_property_read_u32_array(client->dev.of_node, "cal_factor", obj->cal_factor, RATIO_RANGE);
	if (err) {
		APS_ERR("get cal_factor info from dts failed\n");
		obj->cal_factor[0] = 337;
		obj->cal_factor[1] = 374;
		obj->cal_factor[2] = 374;
		obj->cal_factor[3] = 460;
	}
