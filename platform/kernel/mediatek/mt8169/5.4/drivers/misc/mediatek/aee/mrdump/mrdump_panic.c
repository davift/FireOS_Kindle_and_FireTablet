...
static inline void show_kaslr(void)
{
	...
	aee_nested_printf("Kernel Offset: 0x%llx from 0x%lx\n",
			kaslr_off, KIMAGE_VADDR);
	aee_nested_printf("PHYS_OFFSET: 0x%llx\n", PHYS_OFFSET);
	aee_rr_rec_kaslr_offset(kaslr_off);
}
#else
static inline void show_kaslr(void)
{
	aee_nested_printf("Kernel Offset: disabled\n");
...
void mrdump_exception_reboot(void)
{
#if IS_ENABLED(CONFIG_MTK_AEE_TFA)
	aee_exception_reboot(AEE_REBOOT_MODE_WDT);
#else
	aee_exception_reboot();
#endif
}
EXPORT_SYMBOL(mrdump_exception_reboot);
