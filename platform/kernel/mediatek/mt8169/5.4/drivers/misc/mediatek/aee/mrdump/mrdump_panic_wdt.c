...
static void aee_wdt_dump_backtrace(unsigned int cpu, struct pt_regs *regs)
{
	...
	memset(&cur_frame, 0, sizeof(struct stackframe));
	bottom = regs->reg_sp;
...
{
	aee_sram_fiq_log("aee_wdt_zap_locks: weak function\n");
}
...
static inline void show_kaslr(void)
{
	...
	aee_wdt_printf("Kernel Offset: 0x%llx from 0x%lx\n",
			kaslr_off, KIMAGE_VADDR);
	aee_wdt_printf("PHYS_OFFSET: 0x%llx\n", PHYS_OFFSET);
	aee_rr_rec_kaslr_offset(kaslr_off);
}
#else
static inline void show_kaslr(void)
{
	aee_wdt_printf("Kernel Offset: disabled\n");
	aee_rr_rec_kaslr_offset(0xd15ab1e);
}
#endif

extern void mrdump_exception_reboot(void);
