...
static s32 cmdq_sec_check_sec(struct cmdq_pkt *pkt)
{
	...
	if (pkt->sec_data) {
		cmdq_log("pkt->sec_data is not null");
		return 0;
	}
	sec_data = kzalloc(sizeof(*sec_data), GFP_KERNEL);
	if (!sec_data) {
		cmdq_err("kzalloc sec_data failed!");
		return -ENOMEM;
	}
	...
	u32 count, void *meta_array)
{
	...
	size_t size = 0;
	s32 ret = 0;
	struct cmdq_sec_addr_meta *sec_addr_meta = NULL;
	...
	if (!pkt_meta_array) {
		cmdq_err("kzalloc pkt_meta_array failed!");
		return -ENOMEM;
	}
	...
	for (i = 0; ((i < count) && (i < SEC_MAX_HANDLE_NUM)); i++) {
		...
		if (IS_ERR(dma_buffer)) {
			cmdq_err("[%d]MDP secure handle[%llu] get dma_buffer failed[%d]!",
				i, sec_addr_meta->baseHandle, PTR_ERR(dma_buffer));
			for (translate_cnt = 0; translate_cnt < i; translate_cnt++) {
				dma_buf_unmap_attachment(data->dma_attach[translate_cnt],
					data->table[translate_cnt], 0);
				dma_buf_detach(data->dma_buffer[translate_cnt],
					data->dma_attach[translate_cnt]);
				dma_buf_put(data->dma_buffer[translate_cnt]);
			}
			kfree(pkt_meta_array);
			return -EINVAL;
		}
		dma_attach = dma_buf_attach(dma_buffer, pkt->dev);
		if (IS_ERR(dma_attach)) {
			cmdq_err("[%d]MDP secure handle[%llu] get dma_attach failed[%d]!",
				i, sec_addr_meta->baseHandle, PTR_ERR(dma_attach));
			for (translate_cnt = 0; translate_cnt < i; translate_cnt++) {
				dma_buf_unmap_attachment(data->dma_attach[translate_cnt],
					data->table[translate_cnt], 0);
				dma_buf_detach(data->dma_buffer[translate_cnt],
		...
		if (IS_ERR(table)) {
			cmdq_err("[%d]MDP secure handle[%llu] get dma_table failed[%d]!",
				i, sec_addr_meta->baseHandle, PTR_ERR(table));
			for (translate_cnt = 0; translate_cnt < i; translate_cnt++) {
				dma_buf_unmap_attachment(data->dma_attach[translate_cnt],
					data->table[translate_cnt], 0);
				dma_buf_detach(data->dma_buffer[translate_cnt],
		...
		if (dma_addr) {
			...
			cmdq_err("[%d]MDP secure handle[%llu] translate failed!",
				i, sec_addr_meta->baseHandle);
			for (translate_cnt = 0; translate_cnt < i; translate_cnt++) {
				dma_buf_unmap_attachment(data->dma_attach[translate_cnt],
					data->table[translate_cnt], 0);
				dma_buf_detach(data->dma_buffer[translate_cnt],
