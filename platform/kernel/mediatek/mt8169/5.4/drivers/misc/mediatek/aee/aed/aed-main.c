...
#if IS_ENABLED(CONFIG_MTK_AEE_RGU_PMIC_LOG_SUPPORT)
	IPANIC_LAST_RGU_PMIC_LOG = 8,
#endif
};
...
static int kedump_buffer_show_atf(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "\n********last rgu/pmic log********\n");
	memset(&pdata, 0, sizeof(pdata));
	datasize = kedump_get_data_info(IPANIC_LAST_RGU_PMIC_LOG, &pdata);
	if (datasize > 0)
		kedump_add2atf(m, IPANIC_LAST_RGU_PMIC_LOG, pdata, datasize);
#endif
	seq_putc(m, '\0');
