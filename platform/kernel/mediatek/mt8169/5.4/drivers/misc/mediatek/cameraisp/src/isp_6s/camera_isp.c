...
static void ISP_SMI_CG_Dump(char *str, bool smi_dump_en)
{
	...
	LOG_DBG("%s: 4X1_SUB_COMMON_D40C= 0x%x\n",
		...
		enum _isp_dma_enum_ dma_port)
{
	...
	case ISP_CAM_A_INNER_IDX:
		irq_module = ISP_IRQ_TYPE_INT_CAM_A_ST;
		break;
	case ISP_CAM_B_INNER_IDX:
		irq_module = ISP_IRQ_TYPE_INT_CAM_B_ST;
		break;
	case ISP_CAM_C_INNER_IDX:
		irq_module = ISP_IRQ_TYPE_INT_CAM_C_ST;
		break;
	default:
		LOG_NOTICE("unsupported dev node module:0x%x\n", module);
		return;
	}
	IRQ_LOG_KEEPER(irq_module, m_CurrentPPB, _LOG_ERR,
