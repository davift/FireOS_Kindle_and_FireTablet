...
static struct mtee_cma_sec_mem mtee_cma[KREE_SEC_CMA_NUM];
			...
			u8 uparam[REE_SERVICE_BUFFER_SIZE])
{
	...
	struct mtee_cma_sec_mem *cma_sec_mem = NULL;
	int retry;

	/* get parameters */
	chunkmem = (struct ree_service_chunk_mem *)uparam;

#if IS_ENABLED(CONFIG_SVP_DISPLAY_DYNAMIC_SEC_BUF)
	if (!chunkmem || chunkmem->cma_type >= KREE_SEC_CMA_NUM)
		return TZ_RESULT_ERROR_BAD_PARAMETERS;

	cma_sec_mem = &mtee_cma[chunkmem->cma_type];
#else
	cma_sec_mem = &mtee_cma[0];
#endif

	if (cma_sec_mem->secure_pages != NULL || !cma_sec_mem->tz_cma) {
	...
	retry = 10;
	while (retry-- > 0) {
		cma_sec_mem->secure_pages = cma_alloc_large(cma_sec_mem->tz_cma,
					    cma_sec_mem->secure_size / SZ_4K,
					    get_order(SZ_1M));
		if (cma_sec_mem->secure_pages)
			break;
	}

	if (cma_sec_mem->secure_pages == NULL) {
		pr_warn("%s() cma_alloc() failed!\n", __func__);
		return TZ_RESULT_ERROR_OUT_OF_MEMORY;
	}
	chunkmem->size = cma_sec_mem->secure_size;
	chunkmem->chunkmem_pa = (uint64_t)page_to_phys(cma_sec_mem->secure_pages);

	pr_info("%s() get @%llx [0x%zx]\n", __func__,
		chunkmem->chunkmem_pa, cma_sec_mem->secure_size);
			...
			u8 uparam[REE_SERVICE_BUFFER_SIZE])
{
	...
	if (cma_sec_mem->secure_pages != NULL) {
		phys_addr_t addr = page_to_phys(cma_sec_mem->secure_pages);

		cma_release(cma_sec_mem->tz_cma, cma_sec_mem->secure_pages,
			    cma_sec_mem->secure_size >> PAGE_SHIFT);
		pr_info("%s() release @%pax [0x%zx]\n", __func__,
			&addr, cma_sec_mem->secure_size);
		cma_sec_mem->secure_pages = NULL;
		return TZ_RESULT_SUCCESS;
	}
...
static int mtee_probe(struct platform_device *pdev)
{
	...
	mtee_cma[KREE_SEC_CMA_WFD].tz_cma = (struct cma *)rmem->priv;
	mtee_cma[KREE_SEC_CMA_WFD].secure_size = (size_t)rmem->size;
#endif /* CONFIG_WFD_DYNAMIC_SEC_BUF */

#if IS_ENABLED(CONFIG_SVP_DISPLAY_DYNAMIC_SEC_BUF)
	/* get reverved memory address for secure disp application  */
	np = of_parse_phandle(pdev->dev.of_node, "disp-memory-region", 0);
	if (!np) {
		pr_err("No secure disp memory-region\n");
		return -EINVAL;
	}

	rmem = of_reserved_mem_lookup(np);
	if (!rmem) {
		pr_err("failed to get secure disp memory-region\n");
		return -EINVAL;
	}

	of_node_put(np);

	mtee_cma[KREE_SEC_CMA_DISP].tz_cma = (struct cma *)rmem->priv;
	mtee_cma[KREE_SEC_CMA_DISP].secure_size = (size_t)rmem->size;
#endif /* CONFIG_SVP_DISPLAY_DYNAMIC_SEC_BUF */
#endif /* CONFIG_MTEE_CMA_SECURE_MEMORY */
#endif /* CONFIG_OF */
