...
static dma_addr_t translate_fd(struct op_meta *meta,
				struct mdp_job_mapping *mapping_job)
{
	...
	dma_addr_t tmp_result = 0;
	u32 i = 0;
	u32 port = cmdq_mdp_get_hw_port(meta->engine);

	if (!port) {
		CMDQ_ERR("%s get port failed!", __func__);
		return INVALID_MVA;
	}
	...
	if (i == mapping_job->handle_count) {
		...
		if (IS_ERR(dma_buffer)) {
			CMDQ_ERR("MDP handle[%d] get dma_buffer failed[%d]!",
				meta->fd, PTR_ERR(dma_buffer));
			return INVALID_MVA;
		}
		dma_attach = dma_buf_attach(dma_buffer, cmdq_dev_get());
		if (IS_ERR(dma_attach)) {
			CMDQ_ERR("MDP handle[%d] get dma_attach failed[%d]!",
				meta->fd, PTR_ERR(dma_attach));
			dma_buf_put(dma_buffer);
			return INVALID_MVA;
		}
		table = dma_buf_map_attachment(dma_attach, 0);
		if (IS_ERR(table)) {
			CMDQ_ERR("MDP handle[%d] get table failed[%d]!",
				meta->fd, PTR_ERR(table));
		...
		if (dma_addr) {
			if (check_add_overflow(dma_addr,
						(dma_addr_t)meta->fd_offset,
						&tmp_result)) {
				CMDQ_ERR("%s add overflow dma_addr %llu offset %u\n",
					__func__, dma_addr, meta->fd_offset);
			...
			mapping_job->mvas[i] = dma_addr;
			mapping_job->handle_count++;
		} else {
			CMDQ_ERR("%s dma_addr is invalid\n", __func__);
			mdp_ion_free_handle(dma_buffer, dma_attach, table);
			return INVALID_MVA;
		}
	} else {
		dma_addr = mapping_job->mvas[i];
	}
	dma_addr += meta->fd_offset;
			...
			struct cmdq_command_buffer *cmd_buf)
{
	...
	{
		dma_addr_t mva = translate_fd(meta, mapping_job);
...
static s32 translate_user_job(struct mdp_submit *user_job,
			...
			struct cmdq_command_buffer *cmd_buf)
{
	struct op_meta *metas = NULL;
	s32 status = 0;
	u32 i = 0, copy_size = 0, copy_count = 0, remain_count = 0;
	u32 slot_id = 0, j = 0;
		...
		for (i = 0; i < copy_count; i++) {
			...
			if (metas[i].op == CMDQ_MOP_READ || metas[i].op == CMDQ_MOP_READBACK) {
				...
				if (rb_slot[slot_id].ref_cnt == MAX_REF_COUNT) {
					CMDQ_ERR("readback slot [%u] reach maximum ref_cnt\n",
						slot_id);
					mutex_unlock(&rb_slot_list_mutex);
					kfree(metas);
					return -EINVAL;
				...
				for (j = 0; j < ARRAY_SIZE(handle->slot_ids); j++) {
					if (slot_id == handle->slot_ids[j])
						break;
					if (handle->slot_ids[j] == MAX_RB_SLOT_NUM) {
						handle->slot_ids[j] = slot_id;
						rb_slot[slot_id].ref_cnt++;
						CMDQ_MSG("slot id %u, count++ > %u\n",
							slot_id, rb_slot[slot_id].ref_cnt);
						break;
					}
				}
			}
...
s32 mdp_ioctl_async_wait(unsigned long param)
{
	...
	do {
		...
		for (i = 0; i < ARRAY_SIZE(handle->slot_ids); i++) {
			if (handle->slot_ids[i] != MAX_RB_SLOT_NUM) {
				rb_slot[handle->slot_ids[i]].ref_cnt--;
				CMDQ_MSG("slot id %u, count-- by read > %u\n", handle->slot_ids[i],
					rb_slot[handle->slot_ids[i]].ref_cnt);
			}
		}
		mutex_unlock(&rb_slot_list_mutex);
...
s32 mdp_ioctl_alloc_readback_slots(void *fp, unsigned long param)
{
	...
	if (rb_slot[alloc_slot_index].ref_cnt) {
		CMDQ_ERR("%s alloc slot which is being used, slot id: %u\n",
...
s32 mdp_ioctl_free_readback_slots(void *fp, unsigned long param)
{
	...
	if (rb_slot[free_slot_index].ref_cnt) {
		CMDQ_ERR("%s slot id[%u] is in use, using slot count : %u\n", __func__,
