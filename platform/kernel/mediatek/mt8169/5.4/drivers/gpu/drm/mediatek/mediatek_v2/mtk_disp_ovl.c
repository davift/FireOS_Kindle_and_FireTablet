			  ...
			  enum mtk_ddp_io_cmd io_cmd, void *params)
{
	...
	case BACKUP_OVL_STATUS: {
		...
		cmdq_pkt_read(handle, NULL,
			comp->regs_pa + DISP_REG_OVL_STA, CMDQ_THR_SPR_IDX0);

		cmdq_pkt_write(handle, comp->cmdq_base, slot, CMDQ_THR_SPR_IDX0, ~0);
		break;
	}
	default:
		break;
			     ...
			     void *data)
{
	...
	if (IS_ERR(priv->ddp_comp.fbdc_qos_req)) {
		DDPPR_ERR("%s get invalid fbdc_qos_req path!\n", __func__);
		priv->ddp_comp.fbdc_qos_req = NULL;
	}
