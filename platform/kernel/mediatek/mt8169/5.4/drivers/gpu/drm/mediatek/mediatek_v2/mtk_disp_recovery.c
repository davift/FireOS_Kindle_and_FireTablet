...
static DEFINE_MUTEX(esd_recover_lock);
...
static int mtk_drm_request_eint(struct drm_crtc *crtc)
{
	...
	/* In order to make sure eint flow run normally, no need return */
	if (ret)
