...
#if IS_ENABLED(CONFIG_LCD_DUAL_IC_ESD_CHECK)
/* send cmd to switch LCD dual IC */
static void lcd_dual_ic_switch(struct mtk_ddp_comp *comp, void *handle, int num)
{
	unsigned int i = 0;
	unsigned int ic_cmd_list[IC_CMD_MAX_NUM] = { 0 };
	struct mtk_dsi *dsi = container_of(comp, struct mtk_dsi, ddp_comp);
	...
	for (i = 0 ; i < IC_CMD_MAX_NUM ; i++) {
		ic_cmd_list[i] = params->lcm_esd_check_table[num].ic_cmd_list[i];
		if (ic_cmd_list[i] == 0)
			break;
...
int mtk_dsi_esd_read(struct mtk_ddp_comp *comp, void *handle, uintptr_t slot)
{
	...
	for (i = 0 ; i < ESD_CHECK_NUM ; i++) {
		...
		t0.Data1 = 0;
#if IS_ENABLED(CONFIG_LCD_DUAL_IC_ESD_CHECK)
		lcd_dual_ic_switch(comp, handle, i);
#endif
	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
	...
	if (!(dsi->mipi_hopping_sta && adjust_panel_params &&
		...
		if (!fps_chg_index && adjust_panel_params &&
	...
	} else if (adjust_panel_params && cur_panel_params) {
