...
static char sessionDuartion[SESSION_TIME_LEN];
...
static struct delayed_work stylus_active_work;      /* stylus drop down on device */
static struct delayed_work stylus_inactive_work;    /* stylus lift from the device */
static struct mutex stylus_metrics_lock;

static void stylus_metrics_func(struct work_struct *work)
{
	char session_StartTime[SESSION_TIME_LEN] = { 0 };
	char session_EndTime[SESSION_TIME_LEN] = { 0 };
	char session_Duartion[SESSION_TIME_LEN] = { 0 };
	...
	memcpy(session_Duartion, sessionDuartion, SESSION_TIME_LEN);
	...
	if (amazon_logger && amazon_logger->minerva_metrics_log) {
		amazon_logger->minerva_metrics_log(minerva_buf, METRICS_STR_LEN, STYLUS_MINERVA_FMT,
			...
			stylus_tap_counter, session_StartTime, session_EndTime, session_Duartion);
	}
}

static void stylus_active_func(struct work_struct *work)
{
	if (work == NULL)
		return;
...
static void stylus_inactive_func(struct work_struct *work)
{
	if (work == NULL)
		return;
	...
	snprintf(sessionDuartion, SESSION_TIME_LEN,
...
static DECLARE_DELAYED_WORK(stylus_metrics_work, stylus_metrics_func);
static DECLARE_DELAYED_WORK(stylus_active_work, stylus_active_func);
static DECLARE_DELAYED_WORK(stylus_inactive_work, stylus_inactive_func);
#endif
			       ...
			       unsigned int type, unsigned int code, int value)
{
	...
	if (disposition != INPUT_IGNORE_EVENT && type != EV_SYN) {
		...
		if (type == EV_KEY && (code == BTN_DIGI && value == BTN_DIGI_VALUE_DOWN))
			schedule_delayed_work(&stylus_active_work, 0);

		if (type == EV_KEY && (code == BTN_DIGI && value == BTN_DIGI_VALUE_UP))
			schedule_delayed_work(&stylus_inactive_work, 0);
#endif
	}
...
static void __exit input_exit(void)
{
	...
	cancel_delayed_work(&stylus_inactive_work);
	cancel_delayed_work(&stylus_active_work);
	cancel_delayed_work(&stylus_metrics_work);
#endif
}
