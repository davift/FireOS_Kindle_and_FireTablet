...
static void  metrics_log_report_falsepositive(struct liquid *liquid)
{
#if IS_ENABLED(CONFIG_AMAZON_MINERVA_METRICS_LOG)
	int duration_hour = 0;
	ktime_t ktime_now;
	...
	ktime_now =  ktime_get_boottime();
	now_ts = ktime_to_timespec(ktime_now);
	duration_hour = (now_ts.tv_sec - liquid->falsepositive_event_ts.tv_sec) /3600 ;
	if ( (liquid->falsepositive_event_ts.tv_sec == 0) || (duration_hour >= 2)) {
		liquid->falsepositive_event_ts.tv_sec =  now_ts.tv_sec;
		log_counter_to_vitals_v2(ANDROID_LOG_INFO,
			VITALS_LD_GROUP_ID, VITALS_LD_EVENT_SCHEMA_ID,
			"LiquidDetection", "LiquidDetection", "FalsePositiveEvent",
			"adc_mv", liquid->wet_adc, "count",
			NULL, VITALS_NORMAL,
			NULL, NULL);
	}
#endif
...
static void debounce_report_event(struct liquid *liquid, int event)
{
  	switch (event) {
  	case STATE_DRY:
		if (liquid->wet_count && liquid->wet_count < liquid->wet_count_threshold ) {
			liquid->wet_count = 0;
			metrics_log_report_falsepositive(liquid);
			pr_info("FalsePositive.\n");
		} else
			pr_info("No liquid detected.\n");
  		break;
  	case STATE_DRY_TO_WET:
  		liquid->wet_count ++;
		pr_info("Liquid detected %d .\n" , liquid->wet_count);
		if (liquid->wet_count >= liquid->wet_count_threshold){
			liquid->wet_count = 0;
			liquid_is_wet(liquid);
		}
  		break;
	case STATE_WET:
		pr_info("Liquid still on SBU.\n");
		break;
	case STATE_WET_TO_DRY:
...
static void liquid_routine_work(struct work_struct *work)
{
	...
	if (state == TYPE_DRY) {
		...
		if ((liquid_is_in_range(adc1_mv, threshold_l, ADC_MAX_MV)) || (
			...
			if ((liquid_is_in_range(adc1_mv, threshold_l, ADC_MAX_MV)) || (
				 liquid_is_in_range(adc2_mv, threshold_l, ADC_MAX_MV))) {
				liquid->wet_adc = adc1_mv >  adc2_mv ? adc1_mv : adc2_mv;
				debounce_report_event(liquid, STATE_DRY_TO_WET);
				goto skip;
			}
		}
		debounce_report_event(liquid, STATE_DRY);
	} else {
		/* Wet -> Wet */
		if (liquid_is_in_range(adc1_mv, threshold_l, ADC_MAX_MV) ||
			liquid_is_in_range(adc2_mv, threshold_l, ADC_MAX_MV)) {
			debounce_report_event(liquid, STATE_WET);
			goto skip;
		}
		...
		if (liquid_is_in_range(adc1_mv, threshold_h, ADC_MAX_MV) &&
			...
			debounce_report_event(liquid, STATE_WET_TO_DRY);
		else
			debounce_report_event(liquid, STATE_WET);
	}
...
static void liquid_parse_dt(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(np, "wet_count_threshold", &wet_threshold);
	if (ret < 0)
		wet_threshold = 2;
	liquid->wet_count_threshold = wet_threshold;
