...
#if defined(CONFIG_AMAZON_DSP_FRAMEWORK) || defined(CONFIG_AMZN_DSP_FRAMEWORK)
#include "adf/adf_status.h"
#include "adf/adf_common.h"
#endif /* CONFIG_AMAZON_DSP_FRAMEWORK || CONFIG_AMZN_DSP_FRAMEWORK */
...
#define DRIVER_VERSION				"4.1.20230324"
...
static int dbmdx_va_ve_firmware_ready(struct dbmdx_private *p)
{
	...
	if (p->va_ve_chip_enabled) {
...
#ifdef CONFIG_AMZN_DSP_FRAMEWORK
			adfDebug_blockDbg(0, 1);
			pr_info("%s %d: DSP loading block adf debug\n", __func__, __LINE__);
#endif
		...
		dev_info(p->dev, "%s VA_VE FW was loaded successfully\n",
			__func__);

#ifdef CONFIG_AMZN_DSP_FRAMEWORK
		adfDebug_blockDbg(0, 0);
		pr_info("%s %d: DSP run unblock adf debug\n", __func__, __LINE__);
#endif
	...
	if (p->va_chip_enabled) {
		...
		dev_info(p->dev, "%s VA FW was loaded successfully\n", __func__);
			...
			ssize_t *amodel_chunks_size)
{
	...
	for (chunk_idx = 0; chunk_idx < num_of_amodel_files; chunk_idx++) {
		...
		dev_info(p->dev, "%s: %s firmware successfully requested\n",
			__func__, cur_chunk_name);
	...
	do {
		...
		if (nr_samples >= samples_chunk_size) {
			...
			if (nr_samples >= 2*samples_chunk_size)
				/* when have history data limit the read speed lower than 16 times of normal case to avoid overrun */
				sleep_time_us = (u32)(samples_chunk_size * 8 * 1000) /
					p->current_pcm_rate * 1000 / 16;
			else {
				missing_samples = 2*samples_chunk_size -
						nr_samples;
				sleep_time_us =
					(u32)(missing_samples * 8 * 1000) /
					p->current_pcm_rate * 1000;
			...
			u32 sleep_time_us = (u32)(missing_samples * 8 * 1100) /
					p->current_pcm_rate * 1000;
...
#elif IS_ENABLED(CONFIG_AMAZON_METRICS_LOG)
			snprintf(dbmdx_metrics_log_buf, DBMDX_METRICS_LOG_MAX_SIZE,
					"voice_dsp:def:freq_idl_ts=%d;CT;1,freq_run_ts=%d;CT;1,freq_vad_ts=%d;CT;1:NR",
					*freqTs / 1000,
					*(freqTs + 1) / 1000,
					*(freqTs + 2) / 1000);
			ret = log_to_metrics(ANDROID_LOG_INFO, "voice_dsp", dbmdx_metrics_log_buf);
			if (ret)
